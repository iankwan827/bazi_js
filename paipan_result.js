...
class TenGod extends BaziNode {
    constructor(info, context) {
        ...
        this.isSecondary = !!info.isSecondary; // 中气
        this.isYuQi = !!info.isYuQi;          // 余气
...
class BaziContext {
    ...
    // --- 十神存在性判定 ---
    get hasWealth() { return this.getGodMeta('财星').hasNatal; }
    get hasSeal() { return this.getGodMeta('印星').hasNatal; }
    get hasRob() { return this.getGodMeta('比劫').hasNatal; }
    get hasFood() { return this.getGodMeta('食伤').hasNatal; }
    get hasOfficer() { return this.getGodMeta('官杀').hasNatal; }

    // 财星判定
    get isWealthStrong() { return this.getGodMeta('财星').isStrong; } // 力量强
    get isWealthMany() { return this.getGodMeta('财星').isMany; }   // 数量多
    ...
    // 获取特定类型的十神统计 (exists, strong, isMany, yong)
    getGodMeta(category) {
        const matchingGods = this.gods.filter(g => g && (g.category === category || g.godName === category));
        const natalNodes = matchingGods.filter(g => g.pillarIndex < 4);

        const stems = natalNodes.filter(n => n.isStem);
        const mainRoots = natalNodes.filter(n => !n.isStem && !n.isSecondary && !n.isYuQi);
        const midRoots = natalNodes.filter(n => !n.isStem && n.isSecondary); // 中气
        const yuRoots = natalNodes.filter(n => !n.isStem && n.isYuQi);    // 余气

        // --- 1. 判定“旺”(Strength) - 严格按《十神旺衰.md》文档 ---
        // 标准：得月令(旺/相)、或本气根>=1、或中气根>=2
        const hasWangNode = natalNodes.some(g => g.isWang());
        const hasMainRoot = mainRoots.length >= 1;
        const hasTwoMidRoots = midRoots.length >= 2;
        const isStrong = hasWangNode || hasMainRoot || hasTwoMidRoots;

        // --- 2. 判定“多”(Abundance/Many) - 按用户最新指令：两个以上节点原则 ---
        // A. 本气根 >= 2
        const isDuoMain = mainRoots.length >= 2;
        // B. 中气根 >= 2 且透干 (余气不计入此项“多”的判定)
        const isDuoMid = midRoots.length >= 2 && stems.length >= 1;
        // C. 天干虚浮也得有两个 (无任何地支根时，天干>=2)
        const hasAnyRoot = mainRoots.length > 0 || midRoots.length > 0 || yuRoots.length > 0;
        const isDuoFloating = stems.length >= 2 && !hasAnyRoot;

        // 综合判定“多” (用于身弱财多等判定)
        const isMany = isDuoMain || isDuoMid || isDuoFloating;

        return {
            exists: matchingGods.length > 0,
            hasNatal: natalNodes.length > 0,
            isStrong: isStrong, // 力量旺衰
            isMany: isMany,     // 数量多少
    ...
    get isOfficerJi() { return this.isSelfStrong ? false : this.getGodMeta('官杀').isMany; } // Updated to Many for Ji

    get hasWealthControlSeal() { return this.isSealRestrained; }

    get isWealthJi() { return this.isSelfStrong ? false : this.isWealthMany; }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        calculateBazi: window.calculateBazi,
        BaziProcessor: window.BaziProcessor,
        BaziContext: window.BaziContext
    };
}
...
const dayunList = document.getElementById('dayun-list');

// Define global currentData
...
window.renderInteractionsSplit = function(originalInter, dynamicPillars) {
    ...
    if (dynamicPillars && dynamicPillars[0] && dynamicPillars[1]) {
        ...
        if (window.currentData && window.currentData.pillars) {
            const allPillars = [...window.currentData.pillars];
            let dyObj = { gan: dy.gan, zhi: dy.zhi };
            let lnObj = { gan: '', zhi: '' };
            if (ln.gan) {
                lnObj.gan = ln.gan; lnObj.zhi = ln.zhi;
            } else if (ln.ganZhi && ln.ganZhi.length >= 2) {
                lnObj.gan = ln.ganZhi[0]; lnObj.zhi = ln.ganZhi[1];
            }
            allPillars.push(dyObj);
            allPillars.push(lnObj);

            if (typeof window.getDynamicInteractions === 'function') {
                const dynRes = window.getDynamicInteractions(allPillars, [4, 5]);
                if (dynRes) {
...
window.renderPillars = function(data) {
    if (!chartGrid) return;
    chartGrid.innerHTML = '';
    ...
    if (typeof window.getSpecificBaziYear === 'function') {
        nowYear = window.getSpecificBaziYear(new Date());
    }
    let ln = data.selectedLiuNian || (dy ? dy.liuNian.find(l => l.year === nowYear) : null);
    ...
    if (data.interactions && dy && ln) {
        window.renderInteractionsSplit(data.interactions, [dy, ln]);
    }
};

window.renderDaYun = function(data) {
    if (!dayunList) return;
    dayunList.innerHTML = '';
    dayunList.className = 'dayun-scroll-container';

    if (!data.daYunList || data.daYunList.length === 0) return;
        ...
        data.daYunList.forEach(dy => {
            ...
            if (ln) {
                ...
                if (dmIdx >= 0 && tIdx >= 0) {
                    ...
                    if (['正财', '偏财', '才', '财'].includes(godStr)) godColor = '#27ae60';
                    else if (['正官', '七杀', '官', '杀'].includes(godStr)) godColor = '#e74c3c';
                    else if (['食神', '伤官', '食', '伤'].includes(godStr)) godColor = '#2980b9';
                    else if (['正印', '偏印', '印', '枭'].includes(godStr)) godColor = '#9b59b6';
                    else if (['比肩', '劫财', '比', '劫'].includes(godStr)) godColor = '#e67e22';
                }
...
function selectDaYun(dy, data, ln = null) {
    ...
    window.renderPillars(newData);
    window.renderDaYun(newData);
}
